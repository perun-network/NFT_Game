...
export default class erdstallServerInterface {
  ...
  async init(
    ...
    const erdOperatorUrl: URL = new URL(
      (ssl ? "wss://" : "ws://") + config.erdOperatorUrl + "/ws"
    );
    ...
    if (provider == null) {
      throw new Error(
        "Unable to get Account Provider for Ethereum URL: " + ethRpcUrl
      );
    }

    const user = ethers.Wallet.fromMnemonic(
      config.mnemonic,
    ...
    try {
      session = new Session(
        Address.fromString(user.address),
        user.connect(provider) as Signer,
        erdOperatorUrl
      );
  ...
  async mintNFT(): Promise<{ txReceipt: TxReceipt }> {
    ...
    while (true) {
      try {
        ...
        var txReceipt = await (
          await this._session.mint(this.tokenAddress, id)
        ).receipt;
        // Release Mutex
        release();
        return { txReceipt };
        ...
        if (error instanceof Error) {
          ...
          if (error.message.includes("duplicate")) {
            console.log(
              "Server unable to mint " +
                id +
                " due to duplicate NFT ID...Retrying with " +
                this.nextNftID +
                " ..."
            );
            continue;
  ...
  async burnNFTs(nfts: NFT[]): Promise<{ txReceipt: TxReceipt }> {
    if (!this._session) {
      throw new Error("Server session uninitialized");
    }
    try {
      return {
        txReceipt: await (
          await this._session.burn(getAssetsFromNFT(nfts))
        ).receipt,
      };
    } catch (error) {
  ...
  async transferTo(nft: NFT, to: string): Promise<{ txReceipt: TxReceipt }> {
    if (!this._session) {
      throw new Error("Server session uninitialized");
    }
    try {
      return {
        txReceipt: await (
          await this._session.transferTo(
            getAssetsFromNFT(new Array(nft)),
  ...
  async getNFTs(address?: string): Promise<string[]> {
    ...
    if (address) {
      const account = await this._session.getAccount(
  ...
  registerCallbacks(
    ...
    this._session.on("receipt", (receipt: TxReceipt) => {
      if (receipt.tx instanceof Transfer) {
        // Handle transfer transaction issued by transferTo
        transferCallback(
          receipt.tx.sender.toString(),
          receipt.tx.recipient.toString(),
          getNFTsFromAssets(receipt.tx.values)
        );
      } else if (receipt.tx instanceof Trade) {
        // Handle trade transaction
        transferCallback(
          receipt.tx.offer.owner.toString(),
          receipt.tx.sender.toString(),
          getNFTsFromAssets(receipt.tx.offer.offer)
        );
      } else if (receipt.tx instanceof Burn) {
        // Handle burn event
    ...
    this._session.on("exitproof", (balanceProof: BalanceProof) => {
      ...
      transferCallback(
        balanceProof.balance.account.toString(),
        "",
        getNFTsFromAssets(balanceProof.balance.values)
      );
    });
  }
}
